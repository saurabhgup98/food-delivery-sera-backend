...
const sampleRestaurants = [
  ...
  },
  {
    ...
    dishes: [
      ...
      {
        name: 'Rogan Josh',
        description: 'Aromatic lamb curry cooked with yogurt and traditional spices',
        price: '₹520',
        image: 'https://images.unsplash.com/photo-1565557623262-b51c2513a641?w=400&h=300&fit=crop',
        category: 'mains',
        dietary: 'non-veg',
      ...
      },
      {
        ...
        description: 'Cottage cheese in rich, creamy tomato-based gravy',
        price: '₹380',
        image: 'https://images.unsplash.com/photo-1565557623262-b51c2513a641?w=400&h=300&fit=crop',
        category: 'mains',
        dietary: 'veg',
...
export default async function handler(req, res) {
  ...
  // Allow both GET and POST requests for seeding
  if (req.method !== 'GET' && req.method !== 'POST') {
    return res.status(405).json({
      success: false,
      message: 'Method not allowed. Use GET or POST to seed the database.'
    });
  }
  ...
  try {
    ...
    res.status(200).json({
      ...
      data: {
        ...
        dishes: totalDishes,
        details: {
          bangalore: createdRestaurants.filter(r => r.location.city === 'Bangalore').length,
          mumbai: createdRestaurants.filter(r => r.location.city === 'Mumbai').length
        }
      }
    });
